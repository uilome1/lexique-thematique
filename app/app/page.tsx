...
export default function Page() {
    ...
    // -----------------------------------------------------------
    // 3. FONCTIONS ASYNCHRONES / API (Doivent être ici)
    // -----------------------------------------------------------
    ...
    // -----------------------------------------------------------
    // 4. HANDLERS / LOADERS (loadData, handleCreate, etc.)
    // -----------------------------------------------------------
    async function loadData() {
        ...
        try {
            ...
            const dossiersNames: string[] = [];
            
            dossiers.forEach((d: any) => {
                dossiersNames.push(d.nom);
            ...
            if (currentDossier && !newStore[currentDossier]) {
                ...
                setCurrentDossier(dossiersNames[0]);
            }
        } catch (error) {
    ...
    // 5. USE_EFFECTS
    // -----------------------------------------------------------
    // useEffect pour charger au démarrage (VERSION OPTIMALE)
    useEffect(() => {
        // Se déclenche uniquement lorsque l'ID utilisateur est disponible
        if (user?.id) { 
            loadData();
        }
    }, [user?.id]);
    ...
    // -----------------------------------------------------------
    // 6. CHECK IS_LOADED (Correction #418 - doit être AVANT le rendu JSX)
    // -----------------------------------------------------------
    if (!isLoaded) {
        return (
